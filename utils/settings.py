...
def parse_args_inference(dict_args: Union[Dict, None]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip_existing",
        action='store_true',
        help="Skip writing output files if they already exist",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="List of instruments to write (e.g. vocals instrument other instrumental). "
             "Aliases: voice/vocal -> vocals. If omitted, writes the default set for the config/model.",
    )
    parser.add_argument("--lora_checkpoint_peft", type=str, default='', help="Initial checkpoint to LoRA weights")
    parser.add_argument("--filename_template", type=str, default='{file_name}_{instr}',
                        help="Output filename template, without extension. Use '/' in the template to create subdirectories. Default: '{file_name}_{instr}'")
    parser.add_argument("--lora_checkpoint_loralib", type=str, default='', help="Initial checkpoint to LoRA weights")
    if dict_args is not None:
        args = parser.parse_args([])
        args_dict = vars(args)
